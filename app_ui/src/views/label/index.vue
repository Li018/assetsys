<script setup name="user">
  ...
  const handleUpdate = async (id) => {
    ...
    const res = await request.get(`/label/${id}`)
  ...
  const handleBatchDel = async () => {
    ...
    await request.delete(`/label/batch/${ids}`)
    ElMessage({
      showClose: true,
      message: '批量删除成功',
      type: 'success'
  ...
  const load = async () => {
    const res = await request.get('/label/page', {
      params: {
        name: name.value,
  ...
  const beforeBatchUpload = async (file) => {
    let fd = new FormData()
    fd.append('file', file)
    await request.post('/label/batch/upload', fd, {
      headers: {
...
<template>
  ...
  <el-row>
    ...
    <el-col style="margin-bottom: 10px; display: flex; gap: 10px">
      <!--   新增   -->
      <div v-permission="'sys:label:add'">
        <el-button type="primary" :icon="Plus" @click="handleAdd" size="small" plain
        >新增
        </el-button>
      </div>
      <!--   批量导入   -->
      <div v-permission="'sys:label:batch:export'">
      ...
      <!--   批量删除   -->
      <div v-permission="'sys:label:batch:delete'">
        <el-popconfirm
...
<!--        <el-table-column prop="id" label="ID" align="center"/>-->
        <el-table-column prop="name" label="标签名字"  align="center"/>
            ...
            <div style="display: flex; justify-content: center; gap: 10px">
              <div>
                <el-button
                        ...
                        v-permission="'sys:label:update'"
                >编辑
                </el-button>
              </div>
              <div v-permission="'sys:label:delete'">
                <el-popconfirm
                        confirm-button-text="确定"
                        cancel-button-text="取消"
                        :icon="InfoFilled"
