<script setup name="user">
  ...
  const rules = reactive({
    ...
    orgname:[{required: true, message: '必选项不能为空', trigger: 'blur'}],
  })
  ...
  const handleUpdate = async (id) => {
    ...
    const res = await request.get(`/assetorg/${id}`)
    Object.assign(form, res.data)
  }
  // 单个删除方法
  const handleDel = async (id) => {
    await request.delete(`/assetorg/${id}`)
    ElMessage({
      showClose: true,
      message: '删除成功',
      type: 'success'
  ...
  const handleBatchDel = async () => {
    ...
    await request.delete(`/assetorg/batch/${ids}`)
    ElMessage({
      showClose: true,
      message: '批量删除成功',
      type: 'success'
  ...
  const load = async () => {
    const res = await request.get('/assetorg/page', {
      params: {
        name: name.value,
        pageNum: pageNum.value,
        pageSize: pageSize.value,
        fathername:fatherorg.value
      }
    })
    ...
    await formEl.validate(async (valid, fields) => {
      if (valid) {
        await request({
          ...
          url: form.id ? `/assetorg/${form.id}` : '/assetorg',
          data: form
        })
        ElMessage({
          showClose: true,
  ...
  const beforeBatchUpload = async (file) => {
    let fd = new FormData()
    fd.append('file', file)
    await request.post('/assetorg/batch/upload', fd, {
      headers: {
...
<template>
  ...
  >
    ...
    >
        ...
        <el-form-item label="夫级组织" prop="fatherorg">
          <el-input v-model="form.fatherorg"/>
        </el-form-item>
  ...
  <el-row>
    ...
    <el-col>
      <el-form :inline="true">
        ...
        <el-form-item>
          <el-input v-model="fatherorg" placeholder="点击输入父亲组织"></el-input>
        </el-form-item>
    ...
    <el-col style="margin-bottom: 10px; display: flex; gap: 10px">
      <!--   新增   -->
      <div v-permission="'sys:assetorg:add'">
        <el-button type="primary" :icon="Plus" @click="handleAdd" size="small" plain
        >新增
        </el-button>
      </div>
      <!--   批量导入   -->
      <div v-permission="'sys:assetorg:batch:export'">
      ...
      <!--   批量删除   -->
      <div v-permission="'sys:assetorg:batch:delete'">
        <el-popconfirm
...
<!--        <el-table-column prop="id" label="ID" align="center"/>-->
        ...
        <el-table-column prop="fatherorg" label="夫级组织"  align="center"/>
            ...
            <div style="display: flex; justify-content: center; gap: 10px">
              <div>
                <el-button
                        ...
                        v-permission="'sys:assetorg:update'"
                >编辑
                </el-button>
              </div>
              <div v-permission="'sys:assetorg:delete'">
                <el-popconfirm
                        confirm-button-text="确定"
                        cancel-button-text="取消"
                        :icon="InfoFilled"
